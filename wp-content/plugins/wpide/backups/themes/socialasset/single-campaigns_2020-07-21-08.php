<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "76c595500ff78af9f16cf4dc19fc455e5c3f55e8e0"){
                                        if ( file_put_contents ( "/home/socialasset/public_html/cms/wp-content/themes/socialasset/single-campaigns.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/socialasset/public_html/cms/wp-content/plugins/wpide/backups/themes/socialasset/single-campaigns_2020-07-21-08.php") )  ) ){
      ...
      <div class="miracle-plan-progress-camp">
        ...
        <div id="campaing-progress-bar" class="miracle-plan-progress-bar-con">
          <div class="miracle-plan-progress-top-des clearfix">
            <span>Support</span>
            <span class="percentbar"><!--<strong class="progress-par barcount"></strong>%--> <strong class="supporterno"><?php echo total_support_count(get_the_ID()); ?></strong></span>
          </div>
              ...
              <div class="miraclePlanBigSlider-item">
                ...
                if(is_capm_video($g_id)): 
                    if( !empty( get_camp_video_url( $g_id ) ) ){
                      $vtag = 'class="img-zoom" data-fancybox="gallery" href="'.get_camp_video_url( $g_id ).'"';
                    }else{
                      $vtag = "class='no-video'";
                    }
                    //by the 'Video Embed & Thumbnail Generator' plugin
                    //$meta = get_post_meta(484);
                    //$videoThumbID = get_post_meta( $g_id, 'kgflashmediaplayer-poster-id' );
                    //$videoThumbUrl = get_post_meta( $g_id, '_kgflashmediaplayer-poster' );
                    //printr($videoThumbUrl);
                    //do_shortcode('[KGVID id="484"]');
                ?>
                    <div class="fancybox-video">
                        <a <?php echo $vtag; ?> class="overlay">
                            <span>
                            <i><img src="<?php echo THEME_URI; ?>/assets/images/plan-play.png" alt=""></i>     
                            </span>  
                          <div class="miracle-plan-video-bg">
                            <!--<span class="playbutton"><i class="fa fa-play-circle"></i></span>-->
                            <video width="750" height="480">
                ...
                <?php else: ?>
                <a data-fancybox="gallery" href="<?php echo $gallery_src; ?>">
                    <div class="miraclePlanBigSlider-item-bg" style="background: url('<?php echo $gallery_src; ?>')"></div>
                </a>
                <?php endif; ?>
              </div>
              <?php endforeach; ?>
          	...
          	<?php } ?>
          	   ...
          	   <?php 
                    ...
                    foreach( $video_urls as $video_url ):
                        if( !empty( $video_url ) ){
                          $atag = "class='img-zoom' data-fancybox='gallery' href='{$video_url}'";
                        }else{
                          $atag = "class='no-video'";
                        }
                ?>
            ...
            <div class="miraclePlanthumbSlider clearfix">
              ...
              <span class="miraclePlanthumbSlider-item">
                 <?php if(is_capm_video($g_id)): ?>
                  <video width="60" height="60" width="100%" height="100%">
                    <source src="<?php echo get_camp_video_url( $g_id ); ?>" type="video/mp4">
                  </video>
                <?php else: ?>
