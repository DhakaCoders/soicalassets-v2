...
function campaignDraft(id, nonce){
    jQuery.ajax({
        type: 'post',
        url: ajax_draft_camp_object.ajaxurl,
        data: {
            action: 'my_draft_capm',
            nonce: nonce,
            id: id
        },
        success: function( result ) {
    ...
    jQuery.ajax({
        ...
        success: function( result ) {
            ...
            if( typeof(result['success']) != "undefined" &&  result['success'].length != 0 && result['success'] == 'success' ) {
                jQuery(".percentbar strong.barcount").text(result['bar']);
                jQuery(".percentbar .supporterno").text(result['toalsupport']);
                jQuery(".percentwidth span").css("width", result['bar']+"%");
...
/*              var bar = result['bar']+10;
                var toalsupport = result['toalsupport']+2;
                jQuery(".percentbar strong.barcount").text(bar);
                jQuery(".percentbar .supporterno").text(toalsupport);
                jQuery(".percentwidth span").css("width", bar+"%");*/
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            if(typeof(data['user_status']) != "undefined" &&  data['user_status'].length != 0 && data['user_status'] == 'success'){
                ...
                } else if( typeof(data['popup_success']) != "undefined" &&  data['popup_success'].length != 0 ){
                    jQuery('.popupsignup-wrapp #user-signup').hide();
                    jQuery('.popup-success').show();
                    jQuery('.popup-success').html(data['popup_success'])
                }
        ...
        success: function(data){
            ...
            if(typeof(data['login_status']) != "undefined" &&  data['login_status'].length != 0 && data['login_status'] == 'success'){
                ...
                if(typeof(data['login_success']) != "undefined" &&  data['login_success'].length != 0){
                  ...
                  jQuery('#success-login').text(data['login_success']);
                  function redirect_page(){
                     if(typeof(data['login_role']) != "undefined" &&  data['login_role'].length != 0 && data['login_role'] == 'subscriber'){
...
function ownAccountDelete(id){
    if (confirm("Do you want to remove your account?")) {
        // your deletion code
    jQuery.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: ajax_user_delete_account_object.ajaxurl,
        data: {
            id: id,
            nonce: 'deleteaccount',
        ...
        success: function(data){
            ...
            if(typeof(data['status']) != "undefined" &&  data['status'].length != 0 && data['status'] == 'success'){
                function redirect_page(){
                    window.location.href = ajax_user_delete_account_object.redirecturl; 
                }
